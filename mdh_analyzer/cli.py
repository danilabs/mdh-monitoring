...
def generate_report(args):
    ...
    try:
        if not os.path.exists(args.pixel_data):
            print(f"Error: Pixel data file '{args.pixel_data}' not found", file=sys.stderr)
            sys.exit(1)
        
        if not os.path.exists(args.domain_analysis):
            print(f"Error: Domain analysis file '{args.domain_analysis}' not found", file=sys.stderr)
            sys.exit(1)
        ...
        if args.verbose:
            print(f"Generating markdown report from:")
        ...
        # Generate report
        report_file = generator.generate_report(args.pixel_data, args.domain_analysis, output_file)
...
def main():
    ...
    pixel_parser = subparsers.add_parser('pixels', help='Analyze pixel data from homepage')
    ...
    domain_parser.add_argument(
        'input_file',
        help='Path to pixel data JSON file'
    )
    domain_parser.add_argument(
        '--output-dir',
        default='reports',
    ...
    report_parser = subparsers.add_parser('report', help='Generate markdown report from pixel and domain data')
    ...
    report_parser.add_argument(
        'domain_analysis',
        help='Path to domain analysis JSON file'
    )
    report_parser.add_argument(
        '--output-dir',
        default='reports',
