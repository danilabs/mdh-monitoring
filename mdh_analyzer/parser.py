...
class HTMLMapParser:
    ...
    def _load_html(self) -> None:
        ...
        try:
            ...
            raise IOError(f"Could not read HTML file {self.html_file}: {e}") from e
        except Exception as e:
            raise RuntimeError(f"Could not parse HTML file {self.html_file}: {e}") from e
    ...
    def extract_map_areas(self, map_name: str = "Map") -> List[Dict[str, Any]]:
        ...
        if not map_element:
            raise ValueError(f"Map with name '{map_name}' not found")
    ...
    def _parse_area_element(self, area) -> Optional[Dict[str, Any]]:  # pylint: disable=too-many-locals
        """
